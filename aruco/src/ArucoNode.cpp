...
private:
	...
	// double _quad2Cal[9] = { 640,0,360, 0,640,360, 0,0,1}
	// 	, _monoCal[9] = { 320,0,240, 0,320,240, 0,0,1};
	// 	, _quad2Distortion[5] = {0, 0, 0, 0, 0}
	// 	, _monoDistortion[5] = {0, 0, 0, 0, 0};
	Mat _quad2Intrinsic = cv::Mat(3, 3, CV_64F) //, { 640,0,360, 0,640,360, 0,0,1})
		, _monoIntrinsic = cv::Mat(3, 3, CV_64F) //, { 320,0,240, 0,320,240, 0,0,1})
		, _quad2Distortion = cv::Mat(1, 5, CV_64F) //, {0, 0, 0, 0, 0})
		, _monoDistortion = cv::Mat(1, 5, CV_64F) //, {0, 0, 0, 0, 0})
		;
	image_transport::ImageTransport _it;
	image_transport::Publisher _debugImgPub;

	image_transport::Subscriber _quad2sub;
	...
	ros::Subscriber _quad2calSub;
	void onQuad2Cal(const sensor_msgs::CameraInfo &msg) {
		if (_quad2Intrinsic.at<double>(0,0)) {
			return;
		}
		for(unsigned int i = 0; i < 9; i++)
			_quad2Intrinsic.at<double>(i / 3, i % 3) = msg.K[i];
		
		for(unsigned int i = 0; i < 5; i++)
			_quad2Distortion.at<double>(0, i) = msg.D[i];

		if(_show_axis) {
			ROS_WARN("%s instrinsic received", msg.header.frame_id.c_str());
			cout << "Intrinsic: " << _quad2Intrinsic << endl;
			cout << "Distortion: " << _quad2Distortion << endl;
		}
	}
	...
	struct _CamState { Vec3d rvec, tvec; } _quad2State, _monoState;
...
, _quad2calSub(_nh.subscribe("/quad2/camera_info", 1 , &Self::onQuad2Cal, this))
...
, aruco_tf_strobe_(_nh.advertise<std_msgs::Header>("/aruco/tf_strobe", 1, true))
{
...
#if 0
	//Camera instrinsic _quad2Intrinsic parameters
	if(_nh.hasParam("calibration")) {
		...
		for(unsigned int i = 0; i < 9; i++)
			_quad2Intrinsic.at<double>(i / 3, i % 3) = values[i];
	}

	//Camera _quad2Distortion _quad2Intrinsic parameters
	if(_nh.hasParam("distortion"))
	{
		...
		for(unsigned int i = 0; i < 5; i++)
			_quad2Distortion.at<double>(0, i) = values[i];
	}
#endif
...
void Aruco::onQuadFrame(const sensor_msgs::ImageConstPtr& msg) {
	...
	if (_quad2Intrinsic.at<double>(0,0) == 0) {
		ROS_INFO("quad camera not yet calibrated");
		return;
	}
	try {
		...
		aruco::detectMarkers(frame, _board->dictionary, markerCorners, markerIds
			...
			, _quad2Intrinsic, _quad2Distortion
			);
		Vec3d rvec, tvec;
		if(markerIds.size() <= 0
			|| !aruco::estimatePoseBoard(markerCorners, markerIds, _board
					, _quad2Intrinsic, _quad2Distortion //, rvec, tvec
					, _quad2State.rvec, _quad2State.tvec
					// sometimes yields Z axis going INTO the board. so can't use this
					// , cv::SOLVEPNP_P3P
					)) {
			return;
		}
		rvec = _quad2State.rvec; tvec = _quad2State.tvec;
		...
		*/
		...
		if (_show_axis) { // Show the board frame
		  	cv::aruco::drawAxis(frame, _quad2Intrinsic, _quad2Distortion, rvec, tvec, 0.8);
...
void Aruco::onMonoFrame(const sensor_msgs::ImageConstPtr& msg) {
