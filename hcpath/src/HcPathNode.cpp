...
, bumper_sub_(_nh.subscribe("/bumper", 1, &Self::chatterCallback, this))
...
void HcPathNode::chatterCallback(const gazebo_msgs::ContactState &cs) {
  int a;
  a = cs.total_wrench.force.x;
  if (a==0) {
	ROS_INFO_THROTTLE(0.25, "Contact detected");
  }
}